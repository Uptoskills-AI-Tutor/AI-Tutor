...
const RecentActivities.FC<RecentActivitiesProps> = ({ activities }) => {
  const getActivityIcon = (type) => {
    switch (type) {
      case 'quiz' <Zap size={18} />;
      case 'lesson' <BookOpen size={18} />;
      case 'assignment' <FileCheck size={18} />;
  ...
  const getActivityBackground = (type) => {
    switch (type) {
      case 'quiz' 'bg-primary-500';
      case 'lesson' 'bg-success-500';
      case 'assignment' 'bg-warning-500';
      default 'bg-gray-500';
    }
  };
          ...
          <div key={activity.id} className="flex items-start gap-4">
            ...
            {activity.score !== undefined && (
              <div className={`text-sm font-medium px-2 py-1 rounded ${
                activity.score >= 90 ? 'bg-success-100 text-success-800' .score >= 70 ? 'bg-primary-100 text-primary-800' :
